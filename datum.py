...
def datetime():
    ...
    return ([Begr, user, wtage[week], day, month[month_int-1], year, hour, minute, second])

#      Begrüßung, Datum und Uhrzeit werden auf Standardgerät ausgegeben:
if __name__ == "__main__":
    date = datetime()
    print(f"\n{date[0]}, {date[1]}")
    print(f"\nHeute ist {date[2]}, der {date[3]}. {date[4]} {date[5]}. Es ist {date[6]:02d}:{date[7]:02d}:{date[8]:02d} Uhr\n")
